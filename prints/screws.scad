// constants
$screw_tolerance = 0.15; // default screw tolerance
...
function get_screw(name) =
  ...
  let(screws = [
    ...
    ["4-40", 2.9, [5.3, 4.5]],
    ["M3", 3, [6, 4.5]],
    ["6-32", 3.5, [6.65, 2.11]], // NOTE: countersink dimensions not confirmed
    ["M4", 4, [7.5, 2.5]], // NOTE: counter sink dimensions not confirmed
...
// @param invert_countersink whether to countersink the screw at the other end
...
module machine_screw (name, length, countersink = true, invert_countersink = false, tolerance = $screw_tolerance, stretch = 0, $fn = 30) {
  // parameters
  ...
  union() {
    resize([screw_d+2*tolerance+2*stretch, 0, 0])
      cylinder(h=length, d=screw_d+2*tolerance, center=false, $fn=$fn);
    if (countersink && invert_countersink) {
      translate([0, 0, length])
        rotate([180, 0, 0])
        resize([screw_cs[0]+2*stretch, 0, 0])
          cylinder(h=screw_cs[1], d1=screw_cs[0], d2=screw_d+2*tolerance, center=false, $fn=$fn);
    } else if (countersink) {
      resize([screw_cs[0]+2*stretch, 0, 0])
        cylinder(h=screw_cs[1], d1=screw_cs[0], d2=screw_d+2*tolerance, center=false, $fn=$fn);
    }
