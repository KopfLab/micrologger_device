...
void LoggerController::init() {
  ...
  state_variable[2] = 0;
  strcpy(state_status_variable, "{}");
  state_status_variable[2] = 0;
  strcpy(state_controller_variable, "{}");
  state_controller_variable[2] = 0;
  strcpy(state_components_variable, "{}");
  state_components_variable[2] = 0;
  ...
  Particle.variable(STATE_STATUS_VARIABLE, state_status_variable);
  Particle.variable(STATE_COMPONENTS_VARIABLE, state_components_variable);
  Particle.variable(STATE_CONTROLLER_VARIABLE, state_controller_variable);
...
void LoggerController::update() {
    ...
    if (Particle.connected()) {
        ...
        #else
        #error "'PLATFORM_ID' is not a recognized wifi or cellular particle device"
        #endif
...
void LoggerController::addToStateStatusVariableBuffer(char* info) {
  if (state_status_variable_buffer[0] == 0) {
    strncpy(state_status_variable_buffer, info, sizeof(state_status_variable_buffer));
  } else {
    snprintf(state_status_variable_buffer, sizeof(state_status_variable_buffer),
        "%s,%s", state_status_variable_buffer, info);
  }
}

void LoggerController::addToStateControllerVariableBuffer(char* info) {
  if (state_controller_variable_buffer[0] == 0) {
    strncpy(state_controller_variable_buffer, info, sizeof(state_controller_variable_buffer));
  } else {
    snprintf(state_controller_variable_buffer, sizeof(state_controller_variable_buffer),
        "%s,%s", state_controller_variable_buffer, info);
  }
}

void LoggerController::addToStateComponentsVariableBuffer(char* info) {
  if (state_components_variable_buffer[0] == 0) {
    strncpy(state_components_variable_buffer, info, sizeof(state_components_variable_buffer));
  } else {
    snprintf(state_components_variable_buffer, sizeof(state_components_variable_buffer),
        "%s,%s", state_components_variable_buffer, info);
  }
}
...
void LoggerController::postStateVariable() {
  ...
  // original state variable: dt = datetime, s = state information
  ...
  snprintf(state_status_variable, sizeof(state_status_variable), 
    "{\"dt\":\"%s\",\"version\":\"%s\",\"mac\":\"%02x:%02x:%02x:%02x:%02x:%02x\",\"mem\":%lu,\"sls\":%d,\"dls\":%d}",
    date_time_buffer, version, 
    mac_address[0], mac_address[1], mac_address[2], mac_address[3], mac_address[4], mac_address[5],
    System.freeMemory(), state_log_stack.size(), data_log_stack.size());

  // new state controller variable
